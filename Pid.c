...
void SetPIDlimitations (struct pid* myPID , float Pcon , float Icon , float Dcon , float MinOut , float MaxOut , bool authFilter , bool lockingAuth)
	...
	myPID->outMaxLimit =MaxOut; // SUPERIOR SIGNAL LIMIT
	myPID->Dcon =Dcon;
	myPID->Icon =Icon;
	myPID->Pcon =Pcon;
	myPID->authFilter = authFilter; // inhibit filter Derivative
	myPID->convLocking = lockingAuth; // inhibit derivative signal
	}

void printComponents (struct pid* myPID)
	{

	printf ("P:%f I:%f D:%f Input:%f SetPoint: %f Output:%f\n",
	myPID->errorP,myPID->errorI,myPID->errorD,myPID->InputPoint,myPID->SetPoint,myPID->OutputSignal);
...
float pid_calc (struct pid* pid , float dt)
	...
	//----------------------------------------------------------------
	//PROPORTIONAL----------------------------------------------------
	//Is time invariant depends on wanted and read data
	pid->errorP = pid->Kp*(diff); 
	
	if (pid->Pcon !=0)
	{
	pid->errorP = constrain (pid->errorP , -pid->Pcon , pid->Pcon);
	}
	...
	// discart all time values more than TIMELEAP ms
	if (pid->convLocking)
	{	

	if (dt<=TIMELEAP)
		{
		pid->errorD = -pid->Kd*((pid->InputPoint - pid->lastError) / (dt)) ; 
		}	 
	
		// save last error for next loop
		pid->lastError = pid->InputPoint;
	
		//apply low pass filtering
		if (pid->authFilter == true)
		{
		pid->oldD = (1-BETAFILTER)*pid->oldD+BETAFILTER*pid->errorD;
		pid->errorD = pid->oldD; // safe filtered error D
		}
	...
	//INTEGRATIVE-----------------------------------------------------
	//----------------------------------------------------------------
	pid->errorI += pid->Ki * dt * diff; 

	//WINDUP CORRECTION set limits
	...
	pid->errorI = constrain (pid->errorI , -pid->Icon , pid->Icon);
		...
		{ 
		pid->errorI = 0;// reset Integrative if KI = 0	
		}	


	// total supposed output applied with limits
	outVal = constrain((pid->errorP+pid->errorI+pid->errorD),
	pid->outMinLimit , pid->outMaxLimit);
...
// reset the K's
void setKpid (struct pid* pid, float kp ,float ki ,float kd , float gainP ,
float gainI , float gainD)
	{
	pid->Kp = kp*gainP ;
	pid->Ki = ki*gainI ;
	pid->Kd = kd*gainD ;
