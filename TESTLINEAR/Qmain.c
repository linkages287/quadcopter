...
//#include "bmp180.h" /*on board barometer*/
...
void *threadPX4(void *arg); // define thread read PX4 data and PID
...
float mag[3]; // raw mag
float noG_acc[3]; // linear acc
mov_av gyrox , gyroy , gyroz;
...
int main (void) 
	...
	//-----------------------------------------------------------------
	if (DEBUG !=10) {printf ("Set PID limits: load Ks parameters from file\n");}
	...
	if (configure_px4flow(fd2) == false) {printf ("PX4FLOW config error\n");}
	//---------------------------------------------------------------------	
	...
	pthread_create(&pthPX4,NULL,threadPX4,"Start processing... PX4 flow thread"); // thread serial RX PX4
	...
	while (keepRunning) // start main looping for pid calc	
		...
		if ((partial) >= qclock.tick_1ms) // 1 KHZ CYCLE
			...
			dt = ((float)partial / (qclock.tick_1ms))/1000; // value should be around 0.001
				
			IMU_loop(dt); // get imu data acc,gyr,mag

			//DEBUG
			//printf ("x:%.2f y:%.2f z:%.2f dt:%.5f \n",quad.angleX, quad.angleY,quad.angleZ,dt);
				...
				if (quad.diagnostic == true)
					{
						...
						if (datacounter>DIAGTIME)
							{
							pthreatWriteFile (savedDiagnose); // pthreaded
							datacounter= 0;							
...
void *threadPX4(void *arg)
	{

	quad.baroAlt = 0.0f; // barometric altitude
	...
	//running stop on control-C
	while(keepRunning)
	{

	//read barometric altitude
	//RED 1 BYTE AT A TIME 
	// this is supposed to be much faster than SONAR ALT
	...
	// posmiantain >0 means at least one between alt or position is active
	if ((readDataPX4 (fd2, &quad)) && (quad.posMaintain > 0))
	{

	//timer 
	float dt = (float)(quad.px4_dtimer)/65000;
 
	//pass READ data to PID structure before pid math
	Xpitch.InputPoint =  quad.px4_comp_x; // pitch 
	Yroll.InputPoint = quad.px4_comp_y; // roll
	Alt.InputPoint = quad.px4_alt; // altitude 
	
	// altitude or position and altitude maintain

	if ((altFlag == true) && ((quad.posMaintain == 1) || (quad.posMaintain
	== 11))) 
	{
	// set point altitude is equal to read altitude 
	// try to maintain altitude form now
	Alt.SetPoint = quad.px4_alt;
	altFlag = false;
	...
	quad.px4OutA = pid_calc (&Alt, dt); //altitude fix	
	...
	if (quad.posMaintain == 0) // dont apply correction if not engaged
		{
			//reset 3 components value 
			quad.px4_comp_x = 0;
			quad.px4_comp_y = 0;
			quad.px4_alt = 0;
	...
	// altitude contraints limits passed
	// definition in Qmodel.h
	if (Alt.InputPoint > MAXALT )
	{
		Alt.SetPoint = SAFEALT; // set altitude to 1.0m if too higs
	}

	// DEBUG SECTION--------------------------------------------------------
	//printComponents(&Yroll);
	//printf ("DT:%f PYout:%f InputY:%f\n",dt, quad.px4OutY, Yroll.InputPoint);

	//printf (" DT:%f PXo:%f PYo:%f INX:%f INY:%f\n",dt , quad.px4OutX,
	//quad.px4OutY, Xpitch.InputPoint , Yroll.InputPoint );

	//printf ("dt: %f XIN: %f XOUT: %f XP: %f XI: %f\n",dt, Xpitch.InputPoint,
	...
	//printf ("holdingCode:%d, Xo:%f Yo:%f Ao:%f\n",quad.posMaintain, quad.px4OutX, 	quad.px4OutY, quad.px4OutA);
	//---------------------------------------------------------------------


}// end infinite looping 
...
void *threadFuncBLERX(void *arg)
	...
	if(EndSerial==true){// if end serial string reset all data
		...
		{
		  quad.newDirection = true;
		}

		quad.oldDirection = quad.direction;

		quad.armdisarm = armdisarm;
...
void *threadFuncBLETX(void *arg)
	...
	!=savedVal[1]) || ((int)(quad.angleZ) !=savedVal[2]))
		...
		//save XYZ values, i want to to trasmit values only if they are different
...
void PWM_loop (struct pid* pidp, struct pid* pidr, struct pid* pidy) 
	...
	prussdrv_pru_write_memory(PRUSS0_PRU0_DATARAM, 0, percents, 16); // scrive nella shared RAM
...
void IMU_loop (int dt){
		...
		//computer compensated linear acceleration
   		compute_compensated_acc(q, acc,noG_acc);


		//COMPUTER PITCH ROLL YAW FROM QUADRIONS AND WRITE RESULTS IN &att
		computePRY ( q , &att);
